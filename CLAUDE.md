...
- Owner asked for a code review and then "fix all". Fifteen files changed (14 in `src/`, 1 in
  tests), unreleased on top of v0.2.4.
- Correctness: `WicDecoder` disposes its decode stream after `EndInit`; filmstrip loads via
  `FileStream(ReadWrite|Delete)` and bakes EXIF orientation (was locked file + sideways
  thumbnails); refine-to-full-res checks `IsStillCurrent` instead of the stale `_current`
  (a late A could overwrite the B now on screen); single-instance pipe separator `\n` → `\0`
  with the server splitting on both, and the secondary instance drops its mutex handle;
  UI-thread faults now append the full stack to `%APPDATA%\ImageViewer\error.log`; removed
  dead `ImageSaver.TrySetQuery`.
- UX: `FileSystemWatcher` rescan (500 ms debounce, position restore, removed-entry cache
  invalidation, 1.5 s self-write echo suppression); `Ctrl+V` paste (file drop or bitmap via
  temp PNG; H/V flips now require no Ctrl); `F1`/`?` shortcut dialog; touch pinch-zoom and
  two-finger pan; pasted temp files older than 7 days cleaned on startup.
- CLI/packaging: writable list unified on `ImageWriter.NativeExtensions`; `--recursive` on
  list/info/identify/convert/resize/thumb/rotate/flip; `--jobs N` parallel batch with
  GUID-suffixed temp files (parallel testing caught a same-base-name `.tmp` collision);
  publish PDB glob scoped to the two native symbols. New self-test guards
  `ViewTransform`-vs-`Orientation` composition agreement.
- Validation: Debug build 0 warnings, **253 passed / 0 failed** (was 251, +2 new checks),
  fresh-process 48-decode assembly invariant holds, portable publish (258 files, 179.6 MB,
  2 native symbols removed) and `ImageViewer-0.2.4-setup.exe` (58.5 MB) both compile locally.
  `build/` output is gitignored; `app.ico` regenerated byte-identical by the publish.
